	...
	public static TimeBarModel createFlatModel() {
        ...
        try {
			while (resultSet.next()){
				node = new TransactionNode(resultSet.getString(resultSet.findColumn("LSQLTEXT")),resultSet.getInt(resultSet.findColumn("SCN")), resultSet.getTimestamp("NTIMESTAMP#") );
				nodes.add(node);
			}
        ...
        for (int i=0; i<nodes.size(); i++){
        	DefaultRowHeader header = new DefaultRowHeader("SCN " + Integer.toString(nodes.get(i).getSCN()));
    		EventTimeBarRow row = new EventTimeBarRow(header);
        	for(int j=i; j<nodes.size(); j++){
        		if(nodes.get(i).getSCN() == nodes.get(j).getSCN()){
        			
        			start.setDateTime(nodes.get(j).getTimeStamp().getDay(), (nodes.get(j).getTimeStamp().getMonth()+1), nodes.get(j).getTimeStamp().getYear()+1900, nodes.get(j).getTimeStamp().getHours(), nodes.get(j).getTimeStamp().getMinutes(), 0);
        			EventInterval interval = new EventInterval(start.copy().advanceHours(10), start.copy().advanceHours(15));
